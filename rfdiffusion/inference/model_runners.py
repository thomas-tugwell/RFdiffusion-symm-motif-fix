...
class Sampler:
    ...
    def initialize(self, conf: DictConfig) -> None:
        ...
        needs_model_reload = (
            ...
            or conf.inference.ckpt_override_path != self._conf.inference.ckpt_override_path
            if hasattr(self, '_conf') else True
        )
        ...
        if conf.inference.ckpt_override_path is not None:
            ...
            print(
                "WARNING: You're overriding the checkpoint path from the defaults. "
                "Check that the model you're providing can run with the inputs you're providing."
            )
        else:
            if (
                ...
                if conf.contigmap.provide_seq is not None:
                    ...
                    assert conf.diffuser.partial_T is not None, (
        ...
        ), "trb_save_ckpt_path is not the place to specify an input model. Use inference.ckpt_override_path"
    ...
    def assemble_config_from_chk(self) -> None:
        ...
        This throws an error if there is a flag in the checkpoint 'config_dict' that isn't in the
        inference config. This ensures that whenever a feature is added in the training setup,
        it is accounted for in the inference script.
        """
        ...
        for cat in ['model', 'diffuser', 'preprocess']:
            for key in self._conf[cat]:
                try:
                    print(
                        f"USING MODEL CONFIG: self._conf[{cat}][{key}] = "
                        f"{self.ckpt['config_dict'][cat][key]}"
                    )
        ...
        for override in overrides:
            if override.split(".")[0] in ['model', 'diffuser', 'preprocess']:
                print(
                    f'WARNING: You are changing {override.split("=")[0]} from the value '
                    f'this model was trained with. Are you sure you know what you are doing?'
                )
                mytype = type(
                ...
                self._conf[override.split(".")[0]][override.split(".")[1].split("=")[0]] = \
    ...
    def construct_contig(self, target_feats):
        """
        Construct ContigMap class describing the protein to be generated
        """
    ...
    def sample_init(self, return_forward_trajectory=False):
        ...
        Modify signature/body for different initialization based on the config.
        ...
        if self.diffuser_conf.partial_T:
            assert xyz_27.shape[0] == L_mapped, (
                "there must be a coordinate in the input PDB for each residue implied "
                f"by the contig string. length of input PDB ({xyz_27.shape[0]}) != "
                f"length of contig string ({L_mapped})"
            )
            assert contig_map.hal_idx0 == contig_map.ref_idx0, (
                "for partial diffusion there can be no offset between the index of "
                "a residue in the input and the index of that residue in the output"
            )
        ...
        #################################
        seq_t = torch.full((1, L_mapped), 21).squeeze()  # 21 = mask token
        ...
        if self.potential_conf.guiding_potentials is not None:
            if any(
                ...
                assert len(self.target_feats['xyz_het']) > 0, (
                    "If you're using the Substrate Contact potential, you need a ligand in the input_pdb file!"
                )
                het_names = np.array(
                ...
                for pot in self.potential_manager.potentials_to_apply:
                    if hasattr(pot, 'xyz_motif'):
                        pot.diffusion_mask = self.diffusion_mask.squeeze()
                        pot.xyz_motif = xyz_motif_prealign[0, 0]  # shape [L, 27, 3]
    ...
    def _preprocess(self, seq, xyz_t, t, repack=False):
        ...
        xyz_t (L,14,3): coordinates
        ...
        Returns:
            msa_masked, msa_full, seq[None], torch.squeeze(xyz_t,dim=0),
        ...
        if self.preprocess_conf.d_t1d >= 24:  # add hotspot residues
            ...
            if self.ppi_conf.hotspot_res is None:
                print(
                    "WARNING: you're using a model trained on complexes and hotspot residues, "
                    "without specifying hotspots. If you're doing monomer diffusion this is fine"
                )
                hotspot_idx = []
            else:
                hotspots = [(i[0], int(i[1:])) for i in self.ppi_conf.hotspot_res]
                hotspot_idx = []
                for i, resPDB in enumerate(self.contig_map.con_ref_pdb_idx):
                    if resPDB in hotspots:
    ...
    def sample_step(self, *, t, x_t, seq_init, final_step):
        ...
        Args:
            ...
            seq_init (torch.tensor): (L,22) The sequence at the beginning of this timestep
            x_t (torch.tensor): (L,14,3) The residue positions at the beginning of this timestep
            final_step (int): If t == final_step, we do not apply further steps
        ...
        with torch.no_grad():
            (
                msa_prev, pair_prev, px0, state_prev,
                alpha, logits, plddt
            ) = self.model(
                msa_masked,
                msa_full,
...
class SelfConditioning(Sampler):
    ...
    def sample_step(self, *, t, x_t, seq_init, final_step):
        ...
        Args:
            t (int): The timestep that has just been predicted
            x_t (torch.tensor): (L,14,3) The residue positions at the beginning
            seq_init (torch.tensor): (L,22) The sequence at the beginning
            final_step (int): If t == final_step, we do not apply further steps

        Returns:
            px0: (L,14,3) The model's prediction of x0
            x_t_1: (L,14,3) The updated positions of the next step
            seq_t_1: (L,22) The updated sequence
            plddt: (L, 1) Predicted lDDT of x0
        """
        (
            msa_masked,
        ...
        # If t2d has dims >44, we only overwrite the first 44
        t2d[..., :44] = t2d_44
        ...
        with torch.no_grad():
            ...
            # If we have symmetry self-conditioning, symmetrize px0
        ...
        if t > final_step:
            ...
            for attempt in range(max_retries):
                try:
                    # Add a small random perturbation to x_t for this attempt
                    x_t_perturbed = x_t + torch.randn_like(x_t) * 0.05  # adjust scale as needed

                    (
                        msa_masked, msa_full, seq_in, xt_in,
                        idx_pdb, t1d, t2d, xyz_t, alpha_t
                    ) = self._preprocess(seq_init, x_t_perturbed, t)

                    with torch.no_grad():
                        (
                            _, _, px0, _, alpha, _, plddt
                        ) = self.model(
                            msa_masked, msa_full, seq_in, xt_in, idx_pdb,
                    ...
                    # Convert px0 to all-atom representation
                    _, px0 = self.allatom(torch.argmax(seq_in, dim=-1), px0, alpha)
                    px0 = px0.squeeze()[:, :14]
                    ...
                    self._log.info(
                        f'Timestep {t}, input to next step: '
                        f'{seq2chars(torch.argmax(seq_t_1, dim=-1).tolist())}'
                    )
                    break  # motif RMSD was fine — exit retry loop

                except ValueError as e:
                    if "Motif RMSD too high" in str(e):
                        self._log.warning(
                            f"Timestep {t}: Motif RMSD too high on attempt {attempt + 1}/{max_retries}, retrying..."
                        )
                        continue
                    else:
                        raise
                ...
                self._log.error(
                    f"Timestep {t}: Motif RMSD too high after {max_retries} attempts. Skipping design."
                )
...
class ScaffoldedSampler(SelfConditioning):
    ...
    def __init__(self, conf: DictConfig):
        ...
        Two basic approaches here:
            i) Given a block adjacency/secondary structure input, generate a fold
               (in presence/absence of a target).
               Allows easy generation of binders or specific folds,
               or expansion of an input to sample different lengths.

            ii) Provide a contig input + corresponding block adjacency/secondary structure input.
               Allows mixing motif scaffolding + fold-conditioning.
               Adjacency/secondary structure must match exactly in length
               to the contig string.
        """
        super().__init__(conf)
        # initialize BlockAdjacency sampling class
        if conf.scaffoldguided.scaffold_dir is None:
            # We must have per-residue SS if scaffold_dir is not given
            assert any(
                x is not None
                for x in (
    ...
    def sample_init(self):
        """
        Wrapper method for taking secondary structure + adjacency
        and outputting xt, seq_t
        """
        ##########################
        ### Process Fold Input ###
        ##########################
        if hasattr(self, 'blockadjacency'):
            ...
            self.L = 100  # placeholder if none provided
        ...
        if self.contig_conf.contigs is None:
            ...
            if self.target:
                ...
                contig.append(
                    f"{self.target_pdb['pdb_idx'][-1][0]}"
                    f"{start}-{self.target_pdb['pdb_idx'][-1][1]}/0 "
                )
            ...
            assert (
                self.target is None
            ), "Providing a target doesn't match contigs + secondary structure approach."

            # process target + reinitialize potential_manager
    ...
    def _preprocess(self, seq, xyz_t, t):
        ...
        ), "The checkpoint you're using hasn't been trained with SS/block adjacency features"
        assert (
        ...
        if self.target:
            blank_ss = torch.nn.functional.one_hot(
                torch.full((self.L - self.binderlen,), 3),
                num_classes=4
            )
